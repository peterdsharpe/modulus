...
def _run(m, pts, nrm, drv, w, S, Q):
    sp, sn, ss = S
    q, qn, qs = Q
    return m(pts, nrm, drv, w, query_points=q, query_normals=qn, query_scalars=qs,
...
def test_option_validation():
    ...
    with pytest.raises(ValueError):  # support without scalars while n_query_scalars > 0
        m(pts, nrm, drv, w, query_points=Q[0], query_normals=Q[1], query_scalars=Q[2],
...
def test_passive_queries_take_scalars_without_support():
    ...
    with torch.no_grad():
        a = m(pts, nrm, drv, w, query_points=q, query_normals=qn, query_scalars=qs)
        b = m(pts, nrm, drv, w, query_points=q, query_normals=qn, query_scalars=2 * qs)
